...
class TronEnv(BaseEnv):
    ...
    def get_current_observations(self):
        ...
        current_state = torch.concatenate(
            (self.joint_pos,
             ...
             self.sine_clock * 0.25,
             self.cosine_clock * 0.25,
             self.action *  0.25,
             self.vel_cmd,
             ), dim=1)
    ...
    def get_privilege(self):
        ...
        R_foot_contact_force = self.scene["R_contact_sensor"].data.net_forces_w[:, 0, 2].view(-1, 1)
        # #——————————————————————获取额外机器人状态结束————————————————————————————————##
        return torch.concatenate((self.external_body_force/200,
                                          linear_vel/5,
    ...
    def stand_still(self):
        ...
        stand_still_reward *= 1.0 * (self.vel_cmd == 0)
    ...
    def Termination_reward(self):
        ...
        over3 = torch.abs(self.next_body_ori[:, 2].view(-1, 1)) > np.pi / 2
